...
from models.generation_utils2 import build_chat_input, TextIterStreamer
...
class BaichuanForCausalLM(BaichuanPreTrainedModel):
    ...
    def chat(self, tokenizer, messages: List[dict], messages2: List[dict], stream=False,
        ...
        input_ids1 = build_chat_input(self,tokenizer, messages, generation_config.max_new_tokens)
        input_ids2 = build_chat_input(self,tokenizer, messages2, generation_config.max_new_tokens)
        ...
        # =========手动padding 结束======================
        input_ids = torch.LongTensor([input_ids1,input_ids2]).to(self.device)
    ...
    @torch.no_grad()
    def get_inputs_id(self, tokenizer, messages: List[dict], stream=False,
                    generation_config: Optional[GenerationConfig] = None):
        generation_config = generation_config or self.generation_config
        input_ids = build_chat_input(self, tokenizer, messages, generation_config.max_new_tokens)
