...
def repeat_indices(inpit_indice_list=[13,14,15],expand_dim=3):
    res=[]
    for i in inpit_indice_list:
        for j in range(i*expand_dim,expand_dim*(i+1)):
            res.append(j)
    return res
def train_iql_model():
    """
    ...
    normalize_dic = None
    if is_normalize:
        normalize_indices=repeat_indices([13, 14, 15])
...
def train_model_steps(model, replay_buffer, step_num=100000, batch_size=5040):
    for i in range(step_num):
        ...
        next_states=next_states.to("cuda:0")
        terminals=terminals.to("cuda:0")
        # model=model.to("cpu")
        q_loss, v_loss, a_loss = model.step(states, actions, rewards, next_states, terminals,step_num)
