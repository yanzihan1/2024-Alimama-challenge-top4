...
def normalize_reward(training_data,feature_name="reward"):
    """
    对强化学习奖励进行归一化。
    ...
    Returns:
        归一化奖励的Series。
    """
    reward_range = training_data[feature_name].max() - training_data[feature_name].min()
    training_data["normalize_{}".format(feature_name)] = (
                                                training_data[feature_name] - training_data[feature_name].min()) / reward_range
    return training_data["normalize_{}".format(feature_name)]
