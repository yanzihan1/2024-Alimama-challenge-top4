...
class Q(nn.Module):
    ...
    def forward(self, obs, acts):
        ...
        abs_diff = torch.abs(obs_embedding - action_embedding)
        combined_tensor = torch.cat((obs_embedding, action_embedding, abs_diff), dim=-1)
        q = self.FC3(F.relu(self.FC2(F.relu(self.FC1(combined_tensor)))))
        return q
