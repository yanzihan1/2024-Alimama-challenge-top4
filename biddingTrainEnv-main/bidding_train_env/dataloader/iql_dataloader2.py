    ...
    def _generate_rl_data(self, df):
        ...
        for (episode), group in df.groupby(['episode']):
            ...
            for tick in group['tick'].unique():
                ...
                #
                # remainingBudget_c_state_features=state_features['avg_bid_all']/remainingBudget if remainingBudget else 0
                '''
                TODO: 
                新特征 剩余预算/历史平均出价
                历史平均流量价值与当前流量价值  的比率  state_features['avg_marketPrice_all']
                竞争对手的行为和策略，比如竞争对手的平均出价、竞得概率等
