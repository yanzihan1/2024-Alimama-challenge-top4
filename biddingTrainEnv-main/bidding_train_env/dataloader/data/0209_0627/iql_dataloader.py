...
class IqlDataLoader:
    ...
    def _generate_rl_data(self, df):
        ...
        for (episode, agentIndex), group in df.groupby(['episode', 'agentIndex']):
            ...
            #
            last_7_ticks_volume = tick_volume_sum.rolling(window=7, min_periods=1).sum().shift(1).fillna(0).astype(int)
            group['last_7_ticks_volume'] = group['tick'].map(last_7_ticks_volume)

            last_12_ticks_volume = tick_volume_sum.rolling(window=12, min_periods=1).sum().shift(1).fillna(0).astype(int)
            group['last_12_ticks_volume'] = group['tick'].map(last_12_ticks_volume)
            ...
            for col in ['bid', 'marketPrice', 'Reward', 'status', 'pvValue']:
                ...
                if col=='pvValue':
                    group_agg['avg_pv_bid_last_3'] = group_agg['avg_bid_last_3']/group_agg['avg_pvValue_last_3'] if group_agg['avg_pvValue_last_3']!=0 else 0
                # add new feature
                group_agg[f'avg_{col}_last_7'] = group_agg[col].rolling(window=7, min_periods=1).max().shift(1)
                # group_agg[f'avg_{col}_last_12'] = group_agg[col].rolling(window=12, min_periods=1).mean().shift(1)
            ...
            for tick in group['tick'].unique():
                ...
                tick_bids = df[df['tick'] == tick]['bid']
                # group_5_bids = tick_bids.head(5).fillna(0)
                #================ 关于广告主的竞争环境特征========================
                # 考虑竞争对手的综合出价，这个过程会比较慢 计算量较大
                # condition = (df['episode'] == episode) & (df['tick'] == tick) & (df['agentIndex'] != agentIndex)
                # filtered_df = df[condition]
                # average_bid = filtered_df['bid'].mean()
                #还需要考虑: 前三个单位的其他广告主竞争情况
                #============================================================
                ...
                # 没考虑到的点：只考虑小时 没有考虑day的特征
                # 前三个tick的出价是否太短？ 没有用
                # 新加入的(前7个tick平均出价,前7个tick平均流量价格,前7个tick平均流量价值，前7个tick平均奖励，前7个tick平均竞得概率,前7个tick流量总个数)  没有用
                #
                '''
                TODO: 
                ...
                state = (
                    ...
                    # state_features['avg_bid_last_7'],
                    # state_features['avg_marketPrice_last_7'],
                    # state_features['avg_pvValue_last_7'],
                    # state_features['avg_Reward_last_7'],
                    # state_features['avg_status_last_7'],
                    # state_features['last_7_ticks_volume'],
                    #
                    # state_features['avg_bid_last_12'],
                    # state_features['avg_marketPrice_last_12'],
                    # state_features['avg_pvValue_last_12'],
                    # state_features['avg_Reward_last_12'],
                    # state_features['avg_status_last_12'],
                    # state_features['last_12_ticks_volume'],
                )
