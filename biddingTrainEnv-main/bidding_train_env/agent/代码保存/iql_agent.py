import numpy as np
import torch
import pickle

from bidding_train_env.agent.base_agent import BaseAgent
from bidding_train_env.baseline.iql.iql import IQL


class IqlAgent(BaseAgent):
    """
    IQL方法训练的出价智能体
    """

    def __init__(self, budget=100, name="Iql-PlayerAgent", cpa=2,category=0):
        super().__init__(budget, name, cpa,category)

        # 模型加载
        self.model = IQL(dim_obs=48)
        self.model.load_net("./saved_model/IQLtest")

        # Load and apply normalization to test_state
        with open('./saved_model/IQLtest/normalize_dict.pkl', 'rb') as file:
            self.normalize_dict = pickle.load(file)

    def reset(self):
        self.remaining_budget = self.budget

    def action(self, tick_index, budget, remaining_budget, pv_values, history_pv_values, history_bid,
               history_status, history_reward, history_market_price):
        """
        根据当前状态生成出价。

        :param tick_index: 当前处于第几个tick
        :param budget: 出价智能体总预算
        :param remaining_budget: 出价智能体剩余预算
        :param pv_values: 该tick的流量价值
        :param history_pv_values: 历史tick的流量价值
        :param history_bid: 该出价智能体历史tick的流量出价
        :param history_status: 该出价智能体历史tick的流量竞得状态(1代表竞得，0代表未竞得)
        :param history_reward: 该出价智能体历史tick的流量竞得奖励（竞得流量的reward为该流量价值，未竞得流量的reward为0）
        :param history_market_price: 该出价智能体历史tick的流量市场价格
        :return: numpy.ndarray of bid values
        """
        time_left = (24 - tick_index) / 24
        budget_left = remaining_budget / budget if budget > 0 else 0
        # bgtleft_absolute = remaining_budget / 3500

        # 计算历史状态的均值
        historical_status_mean = np.mean([np.mean(status) for status in history_status]) if history_status else 0
        # 计算历史回报的均值
        historical_reward_mean = np.mean([np.mean(reward) for reward in history_reward]) if history_reward else 0
        historical_reward_var = np.var([np.mean(reward) for reward in history_reward]) if history_reward else 0
        # 计算历史市场价格的均值
        historical_market_price_mean = np.mean(
            [np.mean(price) for price in history_market_price]) if history_market_price else 0

        # 计算历史pvValue的均值
        historical_pv_values_mean = np.mean([np.mean(value) for value in history_pv_values]) if history_pv_values else 0
        # 历史调控单元的出价均值
        historical_bid_mean = np.mean([np.mean(bid) for bid in history_bid]) if history_bid else 0

        # Calculate mean of the last three ticks for different history data

        def mean_of_last_n_elements(history, n, last_number=3):
            last_three_data = history[max(0, n - last_number):n]
            if len(last_three_data) == 0:
                return 0
            else:
                return np.mean([np.mean(data) for data in last_three_data])
        def mean_of_last_n_elements_list(history, n, last_number=6):
            last_three_data = history[max(0, n - last_number):n]
            if len(last_three_data) == 0:
                return [0.,0.,0.,0.,0.,0.]
            else:
                curdata=[np.mean(data) for data in last_three_data]
                if len(curdata)<last_number:
                        padding_number=last_number-len(curdata)
                        res=[]
                        for i in range(padding_number):
                            res.append(0.)
                        res=res+curdata
                else:
                       res=[np.mean(data) for data in last_three_data]
                return res

        def get_window_data(curData, window=6):
            res = []
            # res.append([-1. for _ in range(window)])#shift 1
            if len(curData)<window:
                padding_number=window-len(curData)
                for i in range(padding_number):
                    res.append(0.)
                res=res+curData
            else:
                res=res[-6:]
            return res

        last_3_status_mean = mean_of_last_n_elements(history_status, tick_index)
        last_3_reward_mean = mean_of_last_n_elements(history_reward, tick_index)
        last_3_market_price_mean = mean_of_last_n_elements(history_market_price, tick_index)
        last_3_pv_values_mean = mean_of_last_n_elements(history_pv_values, tick_index)
        last_3_bid_mean = mean_of_last_n_elements(history_bid, tick_index)

        week_marketPrice_change = mean_of_last_n_elements(history_market_price, tick_index)
        # # 7 hours

        # last_3_bid_var=var_of_last_n_elements(history_bid, tick_index)
        # last_3_status_var = var_of_last_n_elements(history_status, tick_index)
        # last_3_reward_var = var_of_last_n_elements(history_reward, tick_index)
        # last_3_market_price_var = var_of_last_n_elements(history_market_price, tick_index)
        # last_3_pv_values_var = var_of_last_n_elements(history_pv_values, tick_index)


        current_pv_values_mean = np.mean(pv_values)
        current_pv_values_var=np.var(pv_values)
        current_pv_num = len(pv_values)

        historical_pv_num_total = sum(len(bids) for bids in history_bid) if history_bid else 0
        last_3_pv_num_total = sum(
            len(history_bid[i]) for i in range(max(0, tick_index - 3), tick_index)) if history_bid else 0

        test_state = np.array([
            time_left, budget_left, historical_bid_mean, last_3_bid_mean,
            historical_market_price_mean, historical_pv_values_mean, historical_reward_mean,
            historical_status_mean, last_3_market_price_mean, last_3_pv_values_mean,
            last_3_reward_mean, last_3_status_mean, current_pv_values_mean,
            current_pv_num, last_3_pv_num_total, historical_pv_num_total

            # historical_bid_var, last_3_bid_var,
            # historical_market_price_var, historical_pv_values_var, historical_reward_var,
            # historical_status_var, last_3_market_price_var, last_3_pv_values_var,
            # last_3_reward_var, last_3_status_var
        ])
        # test_state=np.array([element for element in test_state for _ in range(3)])
        state_features={}
        # for col in ['bid', 'marketPrice', 'Reward', 'pvValue']:
        #            last_three_data = history[max(0, n - last_number):n]

        last_6_reward_mean = mean_of_last_n_elements_list(history_reward, tick_index)
        last_6_market_price_mean = mean_of_last_n_elements_list(history_market_price, tick_index)
        last_6_pv_values_mean = mean_of_last_n_elements_list(history_pv_values, tick_index)
        last_6_bid_mean = mean_of_last_n_elements_list(history_bid, tick_index)

        state_features['bid_last_6_list']=last_6_bid_mean
        state_features['marketPrice_last_6_list']=last_6_market_price_mean
        state_features['Reward_last_6_list']=last_6_reward_mean
        state_features['pvValue_last_6_list']=last_6_pv_values_mean


        test_state=[element for element in test_state for _ in range(3)]
        # for col in ['bid', 'marketPrice', 'Reward', 'pvValue']:
        #     for i in state_features[f'{col}_last_6_list']:
        #         test_state.append(i)
        test_state=np.array(test_state)


        def normalize(value, min_value, max_value):
            return (value - min_value) / (max_value - min_value) if max_value > min_value else 0

        for key, value in self.normalize_dict.items():
            test_state[key] = normalize(test_state[key], value["min"], value["max"])

        test_state = torch.tensor(test_state, dtype=torch.float)
        alpha = self.model.take_actions(test_state)
        bids = alpha * pv_values

        return bids
